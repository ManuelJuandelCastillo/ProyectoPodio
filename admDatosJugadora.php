...
if (isset($_POST['guardar-admOK'])) {
    ...
    $sth = $dbh->prepare('update personas set apellidos=:apellido, nombres=:nombre, fecha_nacimiento=:fechaNac, telefono_particular=:particular, telefono_celular=:celular, telefono_emergencias=:emergencia, correo_electronico=:email, domicilio=:direccion, localidad=:localidad, ficha_ok=:ficha, carnet=:podio, carnet_fmv=:fmv, fecha_ticket=:ticket, foto_4x4_ok=:foto, dni_frente_ok=:dniF, dni_dorso_ok=:dniD where documento=:dni');


    $sth->execute([':apellido' => strtoupper($_POST['apellido']), ':nombre' => strtoupper($_POST['nombre']), ':fechaNac' => $_POST['fecha'], ':particular' => $_POST['particular'], ':celular' => $_POST['celular'], ':emergencia' => $_POST['emergencia'], ':email' => $_POST['email'], ':direccion' => strtoupper($_POST['direccion']), ':localidad' => strtoupper($_POST['localidad']), ':ficha'=>$ficha, ':podio'=>$_POST['carnet-podio'], ':fmv'=>$_POST['carnet-fmv'], ':ticket'=>$_POST['ticket'], ':foto'=>$foto4x4, ':dniF'=>$dni_f, ':dniD'=>$dni_d, ':dni' => $dni]);
...
if (isset($_POST['confirmar-pase'])) {
    ...
    $sth = $dbh->prepare('insert into lista_buena_fe (categoria, torneo, nombre_equipo, documento, fecha_alta) values (:categoria, :torneo, :equipo, :dni, :alta)');
    $sth->execute([':categoria' => $categoria, ':torneo' => $torneo, ':equipo' => $equipoNuevo, ':dni' => $dni, ':alta' => date('Y-m-d')]);
        ...
        <form action="<?= $_SERVER['PHP_SELF'] ?>" method="POST">
            ...
            <div class="card-container">
                <div class="form-group form-input">
                    ...
                    <input type="text" value="<?= $campo_data['telefono_particular'] ?>" name="particular">
                </div>
                <div class="form-group form-input">
                    <label for="celular">Celular</label>
                    <input type="text" value="<?= $campo_data['telefono_celular'] ?>" name="celular">
                </div>
                <div class="form-group form-input">
                    <label for="emergencia">Emergencias</label>
                    <input type="text" value="<?= $campo_data['telefono_emergencias'] ?>" name="emergencia">
                </div>
            </div>
            ...
            <div class="card-container">
                ...
                <div class="form-group form-input">
                    ...
                    <input type="text" name="direccion" value="<?= $campo_data['domicilio'] ?>">
                </div>
                <div class="form-group form-input">
                    <label for="localidad">Localidad</label>
                    <input type="text" name="localidad" value="<?= $campo_data['localidad'] ?>">
                </div>
            </div>
    </section>
        ...
        <div class="img-container" id="img-container">
            <div class="img-card">
                ...
                ?>
                    ...
                    <a href="admEliminarFoto.php?dni=<?= $dni ?>&tipo-imagen=foto" class="form-btn btn-subir">eliminar foto</a>
                <?php
                } else { ?>
            ...
            <div class="img-card">
                ...
                ?>
                    ...
                    <a href="admEliminarFoto.php?dni=<?= $dni ?>&tipo-imagen=dni_f" class="form-btn btn-subir">eliminar foto</a>
                <?php
                } else { ?>
            ...
            <div class="img-card">
                ...
                ?>
                    ...
                    <a href="admEliminarFoto.php?dni=<?= $dni ?>&tipo-imagen=dni_d" class="form-btn btn-subir">eliminar foto</a>
                <?php
                } else { ?>
                    <img src="img/dni.png" alt="" style="width:220px">
                <?php
